...
app.post('/api/products', (req, res) => {
  ...
  db.run(`INSERT INTO product (name, price, description, duration) VALUES (?, ?, ?, ?)`,
    [name, price, description, duration], function(err) {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
      }
...
app.put('/api/products/:id', (req, res) => {
  const id = req.params.id;
  const { name, price, description, duration } = req.body;
  db.run(`UPDATE product SET name = ?, price = ?, description = ?, duration = ? WHERE id = ?`,
    [name, price, description, duration, id], function(err) {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
      }
